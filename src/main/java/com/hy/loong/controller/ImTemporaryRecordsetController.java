...
@Controller
@RequestMapping("/imTemporaryRecordset")
public class ImTemporaryRecordsetController {
	...
	@RequestMapping("/saveFromId")
	@ResponseBody
	public List<ImTemporaryRecordset> saveFromId(String str) throws Exception{
		log.info("进入ImTemporaryRecordsetController：saveIm");
		List<ImTemporaryRecordset> Ims= new ArrayList<ImTemporaryRecordset>();
	    //json字符串转换为json对象  
	    JSONObject jsonObject = JSONObject.parseObject(str);   
	    ImTemporaryRecordset im =new ImTemporaryRecordset(jsonObject.getString("imId"),
	    ...
	    buildDocs(Ims);
	    /**查询数据*/  
	    Ims =searcherDocs(jsonObject.getString("fromId"),jsonObject.getString("groupId"));
		return Ims;
	}
...
*/
        ...
        if(groupId!=null && groupId.length()>0){
            scoreDocs =searcher.search(new TermQuery(new Term("groupId", groupId)),20,sort).scoreDocs;
        }else {
            scoreDocs =searcher.search(new TermQuery(new Term("fromId", groupId)),20,sort).scoreDocs;
        }
